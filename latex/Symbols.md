...
- negate $\not\equiv$ `\not\equiv`
- comment $\overset{\text{def}}{=}$ `\overset{\text{def}}{=}`, and see [Comment equation operators](./math/Comment-equation-operators.md)
...
- See [Follow varying typographic conventions with the same input syntax](./math/Follow-varying-typographic-conventions-with-the-same-input-syntax.md)
