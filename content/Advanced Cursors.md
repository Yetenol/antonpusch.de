---  
dg-publish: true  
not-in-use:   
microsoft-id:   
winget-id:   
github-repo: SkepticMystic/advanced-cursors  
github-release-filename:   
website:   
priority:   
link-modportals:   
modportal0-id: advanced-cursors  
thumbnail:   
categories:  
  - Editing  
synopsis: |  
  Use multiple cursors even more powerfully.  
extends-app: "[[Obsidian]]"  
---  
  
Advanced Cursors is a [Obsidian](./Obsidian.md) extension about editing. Use multiple cursors even more powerfully.    
- Install extension via [Comminity plugins](obsidian://show-plugin?id=advanced-cursors), [Webstore](https://obsidian.md/plugins?id=advanced-cursors)  
- Download the [latest release](https://github.com/SkepticMystic/advanced-cursors/releases/latest) from [Github](https://github.com/SkepticMystic/advanced-cursors)  
  
