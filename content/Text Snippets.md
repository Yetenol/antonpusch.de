---  
dg-publish: true  
not-in-use:   
microsoft-id:   
winget-id:   
github-repo: ArianaKhit/text-snippets-obsidian  
github-release-filename:   
website:   
priority: 1  
link-modportals:   
modportal0-id: text-snippets-obsidian  
thumbnail:   
categories:  
  - Editing  
synopsis: |  
  Snippets for faster typing. Allows you to replace text templates, create your own, and expand text shortcuts.  
extends-app: "[Obsidian](./Obsidian.md)"  
---  
  
Text Snippets is a [Obsidian](./Obsidian.md) extension about editing. Snippets for faster typing. Allows you to replace text templates, create your own, and expand text shortcuts.    
- Install extension via [Comminity plugins](obsidian://show-plugin?id=text-snippets-obsidian), [Webstore](https://obsidian.md/plugins?id=text-snippets-obsidian)  
- Download the [latest release](https://github.com/ArianaKhit/text-snippets-obsidian/releases/latest) from [Github](https://github.com/ArianaKhit/text-snippets-obsidian)  
  
  
---  
Sources:  
  
Related:  
  
Tags:  
[Obsidian](./Obsidian.md)