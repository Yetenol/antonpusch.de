---
title: "Values  - Standardize math, numbers, symbols, quantities, money"
date: "2024-07-24T13:25:20.507+02:00"
dg-publish: true
priority: 4
---

- [LaTeX Symbols - Lookup mathematical symbols, operations, relations, and arrows](./latex-symbols.md)
- [Math references - Typeset mathematical equations and expressions](./math-references.md)
- [Vary the style of mathematical symbols - ϖ vs π](Vary%20the%20style%20of%20mathematical%20symbols%20-%20%CF%96%20vs%20%CF%80.md)
- [Currencies and money -  Standardize monetary values, 3 € vs $ 5](./currencies-and-money.md)
- [Numeric values - Typeset (large) numbers, physical quantities, units, money](./numeric-values.md)
- [Define custom mathematical expressions](./define-custom-mathematical-expressions.md)
 

---
Sources:

Related:

Tags:
[LaTeX - Typeset mathematical and scientific notation, handle cross-referencing and citations, and position images according to defined placement rules](./latex.md)
