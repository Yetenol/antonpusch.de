---
title: "Discussions"
date: "2024-12-16T00:00:00.000+01:00"
dg-publish: true
---

- [Return multiple values in C](./return-multiple-values-in-c.md)
- [Monitor setup - Increase screen real estate with an ultrawide or dual monitor setup](./monitor-setup.md)
- [Curved vs flat monitor](./curved-vs-flat-monitor.md)
- [Indent your code using tabs or spaces](./indent-your-code-using-tabs-or-spaces.md)
- [Comments - Express way of intent with code not comments](./comments.md)
- [Which apps should have command palettes or dark mode](./which-apps-should-have-command-palettes-or-dark-mode.md)
- [Moodle - App vs Website](./moodle.md)
- [Game Theory](Game%20Theory.md)
- [Social Norms](Social%20Norms.md)

- [Command Prompt Comparison - PowerShell vs CMD](Command%20Prompt%20Comparison%20-%20PowerShell%20vs%20CMD.md)
- [Bash Comparison - Powershell Core vs Bash](Bash%20Comparison%20-%20Powershell%20Core%20vs%20Bash.md)
- [Window Managment - Fixed panes vs floating windows](Window%20Managment%20-%20Fixed%20panes%20vs%20floating%20windows.md)
- [Which apps should have command pallettes or dark mode](./which-apps-should-have-command-pallettes-or-dark-mode.md)
- [The Errors of TeX [pdf] (1989)](The%20Errors%20of%20TeX%20pdf)
- [Peer to peer vs Server Client model](Peer%20to%20peer%20vs%20Server%20Client%20model.md)
- [IPv6 vs IPv4](IPv6%20vs%20IPv4.md)
- [IDE - Why are they so cluttered with different build tools](IDE%20-%20Why%20are%20they%20so%20cluttered%20with%20different%20build%20tools.md)
- [Composition vs Inheritance](Composition%20vs%20Inheritance.md)
- [Moodle](./moodle.md)
- [Antiviruses Software - Is it still recommended](Antiviruses%20Software%20-%20Is%20it%20still%20recommended.md)

